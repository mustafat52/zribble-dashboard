...
// Month cols available: Jul-2026 → Dec-2028 (30 months)
// Build a human-readable list for the picker
...
export default function InsightsPage() {
  ...
  // Month range filter — default: full range (Jul 2026 → Dec 2028)
  const [fromCol, setFromCol] = useState(MONTH_COLS[0]);   // Jul-2026
  const [toCol,   setToCol]   = useState(MONTH_COLS[MONTH_COLS.length - 1]); // Dec-2028
  ...
  // ── Client map (all contracts, for overlap detection) ─────────────────────
  ...
  // ── Month-range filtered pipeline (for charts + KPIs) ────────────────────
  ...
  async function handleExport() {
    ...
    try {
      ...
      const totalsRow: (string|number)[] = [
        "TOTAL", "", "", "", "", "", "",
        visibleClients.flatMap(([,cs]) => cs).reduce((a,c) => a+c.dealValue, 0),
        "", "",
        ...selectedMonthCols.map((col) =>
          visibleClients.flatMap(([,cs]) => cs).reduce((sum, c) => {
      ...
      const filterRows = [
        ["Filter", "Value"],
        ["Salesperson", selExecs.length ? selExecs.join(", ") : "All"],
        ["Product",     selProducts.length ? selProducts.join(", ") : "All"],
        ["Account Manager", selAMs.length ? selAMs.join(", ") : "All"],
        ["GST Status",  selGST === "all" ? "All" : selGST === "Y" ? "Registered" : "Not Registered"],
        ["Multi-service only", multiOnly ? "Yes" : "No"],
        ["Month range", `${MONTH_OPTIONS.find(m => m.col === fromCol)?.label} → ${MONTH_OPTIONS.find(m => m.col === toCol)?.label}`],
        [""],
        ["Total clients",   totalClients],
        ["Total contracts", totalContracts],
        ["Range pipeline",  totalPipeline],
        ["Export date",     dateStr],
      ];
      const wsFilter = XLSX.utils.aoa_to_sheet(filterRows);
      wsFilter["!cols"] = [{ wch: 20 }, { wch: 50 }];

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "Filtered Data");
      XLSX.utils.book_append_sheet(wb, wsFilter, "Filter Summary");
    ...
    <PageWrapper wide>
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleExport} disabled={exporting}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition-all shadow-card",
              exporting
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <div>
                ...
                <select value={fromCol} onChange={(e) => {
                  setFromCol(e.target.value);
                  if (MONTH_COLS.indexOf(e.target.value) > MONTH_COLS.indexOf(toCol)) setToCol(e.target.value);
                }}
              ...
              <div>
                ...
                <select value={toCol} onChange={(e) => {
                  setToCol(e.target.value);
                  if (MONTH_COLS.indexOf(e.target.value) < MONTH_COLS.indexOf(fromCol)) setFromCol(e.target.value);
                }}
                  className="px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg bg-white text-slate-700 focus:outline-none focus:border-accent">
                  {MONTH_OPTIONS.map((m) => <option key={m.col} value={m.col}>{m.label}</option>)}
                </select>
              ...
              <div className="flex gap-1 mt-4">
                {[
                  { label:"3m",  from:0, to:2  },
                  { label:"6m",  from:0, to:5  },
                  { label:"12m", from:0, to:11 },
                  { label:"All", from:0, to:29 },
                ].map(({ label, from, to }) => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label:"Pipeline",      value:formatCurrency(totalPipeline), sub:`${selectedMonthCols.length}m range`, icon:IndianRupee, color:"text-accent-green", bg:"bg-accent-greenLight" },
