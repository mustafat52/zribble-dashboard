...
import { Search, ChevronDown, ChevronUp, Receipt, IndianRupee, TrendingUp, AlertTriangle, Clock, Trash2 } from "lucide-react";
  ...
  const rows = useMemo(() => {
    ...
    return [...data].sort((a, b) => {
      const map: Record<SortKey, any[]> = {
        ...
        salesperson: [a.salesperson,             b.salesperson],
        month:       [a.year*100+a.month,        b.year*100+b.month],
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white border border-slate-200 rounded-xl px-5 py-4 shadow-card">
        <div className="flex items-center justify-between mb-2">
          ...
          <p className={cn("text-sm font-bold", collectionRate >= 80 ? "text-accent-green" : collectionRate >= 50 ? "text-accent-amber" : "text-accent-red")}>
            {collectionRate}%
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-card">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <p className="text-xs font-semibold text-slate-500">
            {isLoading ? "Loading..." : `${rows.length} renewal entries`}
          </p>
          <p className="text-xs text-slate-400">Click a row to see payment breakdown</p>
        </div>

        {isLoading ? (
          <div className="px-4 py-12 text-center text-sm text-slate-400 animate-pulse">Loading payments...</div>
        ) : rows.length === 0 ? (
                        ...
                        <Td>
                          <div className="flex items-center gap-2">
                            ...
                            <div className="w-12 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <div className={cn("h-full rounded-full", pct >= 100 ? "bg-accent-green" : pct > 0 ? "bg-accent-amber" : "bg-slate-200")}
                                style={{ width: `${pct}%` }} />
                            </div>
                          </div>
                        </Td>
                                  ...
                                  <div key={p.id} className="flex items-center justify-between text-xs bg-white px-3 py-2 rounded-lg border border-slate-200 gap-3">
                                    ...
                                    {canEdit && (
                                      <button
                                        ...
                                        className="flex-shrink-0 p-1 rounded text-slate-300 hover:text-accent-red hover:bg-accent-redLight transition-all disabled:opacity-40">
