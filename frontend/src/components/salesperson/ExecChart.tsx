...
export function ExecChart({ exec }: { exec: string }) {
  ...
  const contracts = getContractsForSalesperson(exec);
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">{year} total: <span className="font-semibold text-slate-600">{formatCurrency(yearTotal)}</span></p>
        </div>
