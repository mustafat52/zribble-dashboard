  ...
  return (
    <div className="flex flex-col gap-4">
      {overdueItems.length>0&&(
        <div className="flex items-center gap-3 px-4 py-3 bg-accent-redLight border border-red-200 rounded-xl">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-accent-red">{overdueItems.length} overdue renewal{overdueItems.length>1?"s":""} across {overdueClients} client{overdueClients!==1?"s":""} — {formatCurrency(overdueItems.reduce((a,i)=>a+i.outstanding,0))} at risk</p>
      ...
      {grouped.map(([exec,execItems])=>{
        ...
        return (
          <Card key={exec}>
            <button onClick={()=>setCollapsed((prev)=>({...prev,[exec]:!prev[exec]}))}
              ...
              {overdueCount>0&&(
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-accent-redLight text-accent-red border border-red-200">{overdueCount} overdue</span>
              )}
