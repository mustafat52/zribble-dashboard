...
export function RenewalTable({ year, month, onMarkPayment }: RenewalTableProps) {
  ...
  const raw = getRenewalsForMonth(year, month);

  const rows = useMemo(() => {
    let data = raw;
    if (execFilter !== "All")  data = data.filter((r) => r.contract.salesperson === execFilter);
  ...
  }, [raw, execFilter, statusFilter, search, sortKey, sortDir]);
        ...
        <div className="flex gap-1 ml-auto">
          {STATUSES.map((s) => (
            <button key={s.value} onClick={() => setStatusFilter(s.value)} className={cn(
              ...
              {s.label}
              {s.value !== "all" && <span className="ml-1 text-slate-400">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-card flex-1">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <div className="flex items-center gap-1 text-xs text-slate-400">
            ...
            {execFilter !== "All" || statusFilter !== "all" || search ? "Filtered" : "All records"}
          </div>
        </div>
