...
interface ExecContractTableProps { exec: string; onMarkPayment: (contractId: string, year: number, month: number) => void; }

export function ExecContractTable({ exec, onMarkPayment }: ExecContractTableProps) {
  const [search,   setSearch]   = useState("");
  const [expanded, setExpanded] = useState<string|null>(null);
  const [sortDir,  setSortDir]  = useState<"asc"|"desc">("desc");
  const contracts = useMemo(()=>getContractsForSalesperson(exec),[exec]);
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">{filtered.length} of {contracts.length} contracts</p>
        </div>
        <div className="flex items-center gap-2">
          <Input placeholder="Search…" value={search} onChange={(e)=>setSearch(e.target.value)} leftIcon={<Search className="w-3.5 h-3.5"/>} className="w-44"/>
      ...
      {filtered.length===0 ? <EmptyState icon={FileText} title="No contracts found"/> : (
        <>
          <Table>
            <THead>
              <Th>Client</Th><Th>Product</Th><Th>AM</Th><Th>GST</Th><Th>Term</Th><Th>Deal Value</Th><Th>This Month</Th><Th>Status</Th><Th></Th>
            </THead>
            <TBody>
              {filtered.map((c)=>{
                const thisMonthRenewal = c.renewalSchedule.find((r)=>r.year===now.year&&r.month===now.month);
                ...
                return (
                  <>
                    <Tr key={c.id} onClick={()=>setExpanded(isExpanded?null:c.id)} className={cn(isExpanded&&"bg-slate-50")}>
                      <Td>
                        <ClientLink clientName={c.clientName} salesperson={c.salesperson} showDot />
                      </Td>
                      ...
                      <Td>{thisMonthRenewal?<span className="font-medium text-slate-600">{formatCurrency(thisMonthRenewal.amount)}</span>:<span className="text-slate-300">—</span>}</Td>
                      <Td>
                        {thisMonthRenewal?<StatusBadge status={thisMonthRenewal.status}/>:
                          nextRenewal?<span className="text-[11px] text-slate-400">{getMonthShort(nextRenewal.month)} {nextRenewal.year}</span>:
                          <span className="text-slate-300 text-xs">—</span>}
                      </Td>
                      <Td>
                        {thisMonthRenewal&&thisMonthRenewal.status!=="collected"&&(
                          <Button size="sm" variant="secondary" onClick={(e)=>{e.stopPropagation();onMarkPayment(c.id,now.year,now.month);}}>
                    ...
                    {isExpanded&&(
                      <tr key={`${c.id}-exp`} className="bg-slate-50/80">
                        <td colSpan={9} className="px-8 py-4">
                          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-3">Full Renewal Schedule ({c.renewalSchedule.length} months)</p>
                          <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-8 gap-1.5">
                            {c.renewalSchedule.map((r)=>(
                              <div key={`${r.year}-${r.month}`}
                                ...
                                style={r.year===now.year&&r.month===now.month?{backgroundColor:color+"10",borderColor:color+"40"}:{}}
                              >
          ...
          <div className="px-5 py-3 border-t border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <span className="text-xs text-slate-400">Total: <span className="font-semibold text-slate-600 ml-1">{formatCurrency(filtered.reduce((a,c)=>a+c.dealValue,0))}</span></span>
          </div>
        </>
      )}
    </Card>
